...
st.caption("Perbandingan ARIMA vs Random Forest | Data bulanan 2015–2025")
...
if fc_all is not None:
    fc_all = to_datetime(fc_all, "date")
...
# pilih kolom prediksi yang ditampilkan
available_pred_cols = [c for c in pred.columns if c.lower().startswith("pred")]
default_models = []
if "pred_arima" in pred.columns:
    default_models.append("pred_arima")
if "pred_rf_tuned" in pred.columns:
    default_models.append("pred_rf_tuned")
...
with colB:
    if "MAPE" in met.columns and "Model" in met.columns:
        fig_mape = px.bar(
            met,
            x="Model",
            y="MAPE",
        ...
        st.info("Kolom 'Model' dan/atau 'MAPE' tidak ditemukan di metrics.csv")

st.divider()

# =========================================================
# SECTION 2: TREND HISTORIS
# =========================================================
...
fig_hist = px.line(
    df_view,
    x="date",
    y="mid_rate_monthly_mean",
    title="USD/IDR Bulanan (Mid Rate)"
)
...
# =========================================================
# SECTION 3: AKTUAL vs PREDIKSI (TEST)
# =========================================================
...
# =========================================================
# SECTION 4: FORECAST 2026–2030 (CSV)
# =========================================================
st.subheader("Forecast USD/IDR 2026–2030 (Out-of-sample)")

if fc_all is None:
    st.warning(
        "File forecast belum ditemukan: data/forecast_2026_2030.csv\n\n"
        "Silakan upload file tersebut ke folder data/ pada repo GitHub."
    )
else:
    # validasi minimal kolom
    required_fc_cols = {"date", "forecast", "lower", "upper"}
    if not required_fc_cols.issubset(set(fc_all.columns)):
        st.error("Format forecast_2026_2030.csv harus memiliki kolom: date, forecast, lower, upper")
    else:
        years_ahead = st.slider("Horizon forecast (tahun)", 1, 5, 5)
        steps = years_ahead * 12

        # ambil sesuai horizon (cut dari awal)
        fc_df = fc_all.sort_values("date").head(steps).copy()

        # plot historis + forecast
        hist_plot = df[["date", "mid_rate_monthly_mean"]].rename(
            columns={"mid_rate_monthly_mean": "value"}
        )
        hist_plot["series"] = "Historis"

        fc_plot = fc_df[["date", "forecast"]].rename(columns={"forecast": "value"})
        ...
        fig_fc = px.line(
            ...
            y="value",
            color="series",
            title=f"Historis vs Forecast ({years_ahead} tahun ke depan)"
        )
...
if fc_all is not None:
    ...
    st.download_button(
        "Download forecast_2026_2030.csv",
        fc_csv,
